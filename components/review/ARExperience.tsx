...
export function ARExperience({ businessData, plan, onComplete }: ARExperienceProps) {
  ...
  const textColor = isLight ? 'text-slate-800' : '{textColor}';
  const textColorMuted = isLight ? 'text-slate-500' : '{textColorMuted}';
    ...
    >
      ...
      <div className="relative z-40 flex flex-col items-center max-w-[320px] w-full text-center p-6">
        <div className="relative mb-6">
          ...
          <motion.div
            className="h-32 min-w-[8rem] max-w-[20rem] bg-white/10 backdrop-blur-md border border-white/20 rounded-[2rem] p-4 shadow-[0_0_30px_rgba(0,0,0,0.3)] flex items-center justify-center relative z-10 overflow-hidden"
            initial={{ scale: 0.4, opacity: 0 }}
            animate={{ scale: 1, opacity: 1 }}
            transition={{ type: 'spring', stiffness: 200, damping: 15 }}
          >
            {businessData.logo || businessData.logoUrl ? (
              <img src={businessData.logo || businessData.logoUrl} alt="Logo" className="h-full w-auto object-contain drop-shadow-md" />
            ) : (
        ...
        >
          <h2 className="text-2xl font-black {textColor} mb-2 leading-tight uppercase tracking-wide">{businessData.name}</h2>
