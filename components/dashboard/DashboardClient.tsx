...
export function DashboardClient({ initialTab = 'overview' }: { initialTab?: 'overview' | 'qr' | 'analytics' | 'reviews' | 'subscription' }) {
  ...
  const [activeTab, setActiveTab] = useState<'overview' | 'qr' | 'analytics' | 'reviews' | 'subscription'>(initialTab);
    ...
    <div className="min-h-screen bg-[#F8FAFC] flex font-sans dashboard-root" suppressHydrationWarning>
      ...
      <aside className="w-64 bg-white border-r border-slate-200 flex flex-col p-6 shadow-sm h-screen overflow-y-auto sticky top-0">
        ...
        <nav className="flex-1 space-y-1">
          {[
            { id: 'overview', icon: LayoutDashboard, label: 'Overview', action: () => router.push('/dashboard') },
            { id: 'qr', icon: QrCode, label: 'My QR Code', action: () => router.push('/dashboard') },
            { id: 'onboarding', icon: Settings, label: 'Onboarding Setup', action: () => router.push('/onboarding') },
            { id: 'analytics', icon: BarChart3, label: 'Analytics', action: () => router.push('/analytics') },
            { id: 'reviews', icon: MessageSquare, label: 'Reviews', action: () => router.push('/reviews') },
            { id: 'theme', icon: Palette, label: 'Theme Design', action: () => router.push('/onboarding?step=5') },
            { id: 'subscription', icon: Settings, label: 'Subscription', action: () => router.push('/subscription') },
          ].map((item) => (
            <button 
              key={item.label}
              onClick={item.action}
      ...
      <main className="flex-1 p-10 overflow-y-auto relative">
        ...
        {activeTab === 'subscription' && (
          <div className="max-w-4xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
            <h1 className="text-4xl font-black text-slate-900 tracking-tight">Subscription Details</h1>
            <div className="bg-white p-8 rounded-3xl shadow-sm border border-slate-200">
              <div className="flex justify-between items-start mb-8">
                <div>
                  <h2 className="text-2xl font-bold text-slate-900 mb-2">Current Plan: <span className="capitalize text-[#1D9E75]">{user.plan}</span></h2>
                  {user.plan === 'trial' && (
                    <p className="text-slate-500 font-medium text-lg">Trial ends in {analyticsSummary?.trial_days_left || 7} days</p>
                  )}
                </div>
                {(user.plan === 'trial' || user.plan === 'basic') && (
                  <button onClick={() => openUpgradeModal('premium')} className="px-6 py-3 bg-slate-900 text-white rounded-xl font-bold hover:bg-slate-800 transition-all">
                    Upgrade Plan
                  </button>
                )}
              </div>
              <div className="grid grid-cols-2 gap-6">
                <div className="p-6 bg-slate-50 rounded-2xl border border-slate-100">
                  <h3 className="font-bold text-slate-700 mb-2">Billing Cycle</h3>
                  <p className="text-slate-900 text-xl font-black">Monthly</p>
                </div>
                <div className="p-6 bg-slate-50 rounded-2xl border border-slate-100">
                  <h3 className="font-bold text-slate-700 mb-2">Next Payment</h3>
                  <p className="text-slate-900 text-xl font-black">
                    {user.plan === 'trial' ? 'Upgrade required' : 'Oct 24, 2026'}
                  </p>
                </div>
              </div>
            </div>
        ...
        {activeTab === 'reviews' && (
          <div className="max-w-4xl mx-auto space-y-8 flex flex-col items-center justify-center pt-20 animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            <h1 className="text-3xl font-black text-slate-900 tracking-tight text-center">Reviews Portal Coming Soon</h1>
            <p className="text-slate-500 text-center max-w-md">We're building a dedicated portal where you can respond to AI-generated and Google reviews directly from your dashboard.</p>
          </div>
        )}
        ...
        {(activeTab === 'overview' || activeTab === 'qr') && (
          <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
              ...
              <div className="col-span-2 space-y-8">
                <div className="bg-white p-8 rounded-[2.5rem] border border-slate-200 shadow-sm relative overflow-hidden">
                   ...
                   <div className="mt-10 grid grid-cols-2 gap-4">
                    <div className="p-6 bg-slate-50 rounded-2xl border border-slate-100">
                      <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">AI Review Config</p>
                      <p className="font-bold text-slate-900">{b.ai_variant_count || 3} Variants • {b.review_language || 'English'} • {b.theme || 'Classic'} Theme</p>
                    </div>
            ...
            <div className="bg-white p-6 rounded-[2.5rem] border border-slate-200 shadow-sm col-span-2 mt-8">
              <h3 className="font-bold text-slate-900 mb-4">Your Menu Items</h3>
              <div className="flex flex-wrap gap-2">
                {b.menu_items && b.menu_items.length > 0 ? (
                  ...
                  <p className="text-sm text-slate-500">No menu items added during onboarding.</p>
                )}
              </div>
            </div>
        ...
        {activeTab === 'analytics' && (
          <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 space-y-8">
            <div className="flex justify-between items-center mb-10">
              <h1 className="text-4xl font-black text-slate-900 tracking-tight">Analytics & Insights</h1>
            </div>
